...
class GUI:
    ...
    def create_classification_window(self):
        ...
        self.exercises_list.bind('<<ListboxSelect>>', self.on_exercise_selected) # <-- AGGIUNGI QUESTA RIGA
    ...
    def on_exercise_selected(self, event):
        """
        Questa funzione viene eseguita quando l'utente clicca su un esercizio nella lista.
        """
        # Prende il nome dell'esercizio selezionato
        selected_indices = self.exercises_list.curselection()
        if not selected_indices:
            return
        
        selected_exercise = self.exercises_list.get(selected_indices[0])
        
        # Comunica alla classe Classification quale esercizio iniziare
        self.classification.set_exercise(selected_exercise)
        
        # Aggiorna subito l'interfaccia per dare un feedback all'utente
        self.trainer_text.config(text=f"Pronto per iniziare: {selected_exercise}!")
        self.exercise_name.config(text=selected_exercise)
        self.repetitions_value.config(text="0")
